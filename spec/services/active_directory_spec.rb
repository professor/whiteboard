...
describe "ActiveDirectory" do
  ...
  it 'ldap_distinguished_name method returns valid distinguished name for staff' do
    @active_directory_service.ldap_distinguished_name(@faculty_frank).should eq("cn=Faculty Frank,ou=Staff,ou=Sync,dc=cmusv,dc=sv,dc=cmu,dc=local")
  end
  ...
  it 'ldap_distinguished_name method returns valid distinguished name for student' do
    ...
    @active_directory_service.ldap_distinguished_name(@student_sam).should eq("cn=Student Sam,ou=SE,ou=Students,ou=Sync,dc=cmusv,dc=sv,dc=cmu,dc=local")
  end
