...
#      subject.formatted_teams(teams).should == "Team Awesome, Team Beautiful"
  ...
  describe "recently_logged_on_users" do
    before(:each) do
      @faculty_frank.last_sign_in_at = 1.day.ago
      @faculty_frank.save

      @faculty_fagan.last_sign_in_at = 1.month.ago
      @faculty_fagan.save

      @admin_andy.last_sign_in_at = 1.year.ago
      @admin_andy.save
    end
    it "should return only recently signed on users" do
      User.recently_signed_on_users.count should = 2
    end
  end
