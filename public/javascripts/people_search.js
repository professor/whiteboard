...
$(document).ready(function() {
    ...
    ***************************************************/
    ...
    $('#key_contacts_table').on("click", "tbody tr", function () {
        window.location = $(this).find('a').attr('href');
    });
    // apply on people_table
    $('#people_table').on("click", "tbody tr", function(){
        window.location = $(this).find('a').attr('href');
    });
...
function buildSearchResults(search_results_json) {
    if(search_results_json != ''){
        if(photobook_toggled){
            var $result_row_as = [];
            var $photobook_results_main = $('#photobook_results_main');
            // build row number i
            ...
            for (var i in search_results_json){
                if(search_results_json[i].priority)
                    $result_row_as.push(buildResultRowPhotoBookFormat(search_results_json[i]));
            }
            for (var i in search_results_json){
                if(!search_results_json[i].priority)
                    $result_row_as.push(buildResultRowPhotoBookFormat(search_results_json[i]));
            }
            $result_row_as.forEach(function(element){
                $(element).appendTo($photobook_results_main);
            });
            ...
            for (var i in search_results_json){
                if(search_results_json[i].priority)
                    $result_row_trs.push(buildResultRowListFormat(search_results_json[i]));
            }
            ...
            var tbody = $people_table.find('tbody');
            $result_row_trs.forEach(function(element){
                $(element).appendTo($people_table);
            });
            $people_table.find('tbody').trigger("update");
        }
    }
}
...
function buildResultRowPhotoBookFormat(search_results_json){
    var photobook_item_a = $(loadImage(search_results_json.image_uri))
