...
#  layout 'cmu_sv_no_pad', :only => [:index, :show]
    ...
    users.each do |user|
      ...
      if (!user.emailed_recently(:effort_log))
        if ((effort_log.nil? || effort_log.sum == 0)&&(!user.emailed_recently(:effort_log)))
          ...
          create_update_effort_log_warning_email_send_it(user.id)
  ...
  def create_update_effort_log_warning_email_send_it(id)
    user = User.find_by_id(id)
    EffortLogMailer.update_effort_log_warning(user).deliver
  end
...
#    end
  ...
  def create_grace_period_expiring_warning_email_for_course(course_id)
    year = Date.today.cwyear
    week_number = Date.today.cweek
    ...
    users.each do |user|
      ...
      user.effort_log_warning_email = Time.now
      create_grace_period_expiring_warning_email_send_it(user.id)
      user.save
    end
  end

  def  create_grace_period_expiring_warning_email_send_it(id)
    user = User.find_by_id(id)
    EffortLogMailer.grace_period_expiring_warning(user).deliver
  end


  def create_grace_period_expiring_warning_email
    if (!EffortLog.log_effort_week?(Date.today.cwyear, Date.today.cweek))
      #We skip weeks that students aren't taking courses
      logger.info "There is no class this week, so we won't remind students to log effort"
      return
    ...
    courses = Course.remind_about_effort_course_list
    courses.each do |course_id|
      create_grace_period_expiring_warning_email_for_course( course_id)
    end
