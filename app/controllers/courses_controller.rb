...
#    @courses = Course.find(:all, :conditions => ["semester = ? and year = ?", @current_semester.to_s, Date.today.year], :order => "name ASC")
    @courses = Course.find(:all, :conditions => ["semester = ? and year = ?", @current_semester, Date.today.year.to_s], :order => "name ASC")
    index_core
  end
